...
# filtering the data after fetching it
...
# figuring out which screen type was used
def get_screen(screen_pos):
    screens = ['Screen Out', 'Screen Out', 'CNT#stretched', 'CNT#high-purity', 'Glassy Carbon', 'Chromox Ref']
    screen_motor_positions = np.array([0, 13867, 27200, 40533, 53866, 67222]) ## may need to manually change these according to the new BTV's specs
    idx = np.argmin(np.abs(screen_motor_positions - screen_pos))
    return screens[idx]

# creating a gaussian function according to x
def gaus1d(x, a, x0, sigma, c) -> np.ndarray:
    return a * np.exp(-(x - x0) ** 2 / (2 * sigma ** 2)) + c

# ensures that the time zone of the time stamp is correct
def unix_ts_to_dt(ts: int):
    return pd.to_datetime(ts, unit='us', utc=True).tz_convert('Europe/Brussels')

## prints the correct date and time stamp after the conversion
def print_unix_ts(ts: int) -> str:
    return f'{unix_ts_to_dt(ts):%y-%m-%d %H:%M:%S.%f}'

## this makes a subscription parameter with the info about the device
...
## subscribing to updates of the filter and screen of the BTV and leting you know if you're in the current version ??
...
## same thing as above but as an additional check for a secondary device
...
## acquiring the basic info about the device
class Device:
    """Class for a generic (non-ppm) device"""
    acq_ts: int = int(datetime.utcnow().timestamp()*1e6) ## If Python is updated to version 3.12 or anything newer then this should change
    selector = ''
...
## acquiring the basic info and extra info about a PPM (pulse position modulation) device
...
## acquiring the basic info and extra info about a BTV device including the image it picked up
...
class BtvDigitalCamera2(Device):
    """Class for BtvDc"""
    img: np.ndarray = np.zeros((2, 2))
    img_h: np.ndarray = np.zeros(2)
    img_v: np.ndarray = np.zeros(2)
...
## acquiring info about the BTVI device including the image it picked up using the info from the PPM device -- the call to this function is commented out so i'm commenting the function out for ease of use
...
## acquiring the info about a Beam Current transformer -- this is relient on the PPM Device info - the parts that deal with the bunch intensity can be removed in your version
...
## acquiring the info about a Beam Position Monitor -- this is relient on the PPM Device info
...
## acquiring the info about a Beam Quality Monitor which is relient on the PPM Device info -- I can't find the bunch length stuff in the vistar but I think it leads to the beam position stuff
...
## acquiring info about the Beam Loss Monitor using the info about the PPM device and plotting the beam loss results
...
## checking the quality of the SPS beam -- using one of their devices ??
...
## FigureCanvas is a form of backend plotting throught matplotlib
## creating the vistar screen and labeling all of the plots it contains
class VistarCanvas(FigureCanvas):
    ...
    def __init__(self, img_filter_set=0, width=5, height=4, dpi=100, projection: bool = False, parent=None):
        ...
        gs = gridspec.GridSpec(3, 2) ## make this (2,2) to remove the bottom 2 plots then adjust the rest of this class as necessary
        ...
        self.axes = [ax0, ax1, ax2, ax3, ax4], ax5, ax6, ax7
        self.btv_title = self.axes[0].set_title('BTV.XXXX', fontweight='bold')
        self.btv_title2 = self.axes[5].set_title('BTV.XXXX', fontweight='bold')
        self.fig = fig
    ...
    def format_plot(self):
        ...
        self.axes[6].set_title('Horizontal Projection', fontweight='bold')
        self.axes[7].set_title('Vertical Projection', fontweight='bold')
        self.axes[5].set_ylabel('Vertical (mm)')
        self.axes[5].set_xlabel('Horizontal(mm)')
        self.axes[6].set_xlabel('Horizontal (mm)')
        self.axes[6].set_ylabel('Amplitude (a.u.)')
        self.axes[7].set_xlabel('Vertical (mm)')
        self.axes[7].set_ylabel('Amplitude (a.u.)')
    ...
    def update_plot(self,
        ...
        self.update_imshow(h2, v2, img2)
        self.update_projections(h2, v2, img2)
    ...
    def update_bpms(self, bpms: List[BeamPositionMonitor]): ## i think this updates the beam position plot
        ax = self.axes[-1]
    ...
    def update_bcts(self, bcts: List[BeamCurrentTransformer]): ## i think this helps update the beam intensity plot
        ax = self.axes[-2]
...
## printing warnings and what devices were triggered by the beam
def print_triggers(ppm_devices: List[Union[BeamCurrentTransformer, BeamPositionMonitor, BeamQualityMonitor]],
                   non_ppm_devices: List[Union[BtvDigitalCamera, BtvDigitalCamera2]],
...
## creating a dataset where the BTV data will be placed -- why use random numbers ??
class ExtractionFaker(QObject):
    ...
    def run(self, Application: 'HrmtVistar'):
        while True:
            ...
            BtvDevice2 = Application.BtvDevices[1]
            ...
            h2, v2, img2 = self.create_btv_dataset()
            BtvDevice2.img_h = h2
            BtvDevice2.img_v = v2
            BtvDevice2.img = img2
...
## gets the data from when the beam info was taken and about this beam
def get_extraction_event(BctDevices: List[BeamCurrentTransformer],
                         BTV: BtvDigitalCamera,
                         BTV2: BtvDigitalCamera2,
    ...
    Event['BTV2.acqStamp'] = BTV2.acq_ts
    ...
    Event['BTV2.screen'] = BTV2.screen
    Event['BTV2.filter'] = BTV2.od_filter
    ...
    for name, value in zip(fitparams_names, fitparams_values):
        ...
        Event[f'BTV2.fit.{name}'] = value

    return Event


## turning the name of the optics component which was defined in the get_extraction_event function (line 801) into a string
...
## linking the vistar to the location of the logbook in the project directory
class Logger:
...
## acquiring the parameters from the gaussian fit algorithm
...
## using the fitting parameters from the get_fitparams function to print the title of the vistars page
...
## Testing the measurement and its cycles before connecting to and sending data to the logbook
class HrmtVistar(QMainWindow):
    update_signal = pyqtSignal(name='update_signal')
    ExtractionFlag = False

    ## testing to see if the data were fully extracted
    def extraction_test(self, BtvDevice: BtvDigitalCamera, BtvDevice2: BtvDigitalCamera2, BctDevice: BeamCurrentTransformer):
        self.thread = QThread()
    ...
    ## starting the reset time to ensure that the vistar keeps updating with new data
    def reset_timer(self):
        self.update_timer.start(500)

    ## triggering the end of the measurement cycle, extracting the final set of overall data, and moving these data to the logbook
    def cycle_end(self):
        cycle_ts = self.BctDevices[0].cycle_ts
        acq_ts = self.BtvDevices[0].acq_ts
        # acq_ts2 = self.BtvDevices[1].acq_ts ##NECESSARY?

        print_triggers(self.cycled_devices, self.non_cycled_devices, self.BctDevices)
        # BTV Business
        BeamTelevision = self.BtvDevices[0]
        BeamTelevision2 = self.BtvDevices[1]
        h, v, img = BeamTelevision.get_img()
        h2, v2, img2 = BeamTelevision2.get_img()
        ...
        self.canvas.btv_title2.set_text(
            f'{self.BtvDevices[1].name} ({self.BtvDevices[1].screen}/{self.BtvDevices[1].od_filter})'
        )
        
        self.canvas.update_plot(h, v, img, h2, v2, img2, self.BctDevices, self.BpmDevices)
        ...
        # fitParams2 = self.canvas.add_fit(h2, v2, img2)
        title = get_title(extraction=self.ExtractionFlag,
                          spsqc=self.SPSQC,
                          bqm=self.BQM,
                          bct=self.BctDevices[0],
    ...
    ## opening and connecting to the logbook
    ...
    def send_to_logbook(self, ExtractionEvent: dict, fitParams: dict, BLM: BeamLossMonitor, fname: str):
        ...
        btv2_screen = ExtractionEvent['BTV2.screen']
        btv2_filter = ExtractionEvent['BTV2.filter']
        ...
        if sx == 0 or sy == 0:
            ...
            line5 = '<br><strong>BTV Beam sigma and position measurement:</strong>\n' \
                    f'Screen type: {btv_screen} and {btv2_screen}\n' \
                    f'Filter setting: {btv_filter} and {btv2_filter}\n' \
            ...
            line5 = f'''<br>BTV Beam sigma and position measurement: <strong>{btv_screen} / {btv_filter} </strong> and <strong>{btv2_screen} / {btv2_filter} </strong>
    ...
    def __init__(self,
        ...
        dpi = 100
        self.canvas = VistarCanvas( ## this is where the vistar canvas info is defined -- height and width unsure about location
        ...
        self.BtvDevices = [ ## would need to change the BTV name and info here
            BtvDigitalCamera(
            ...
            #)
            BtvDigitalCamera2(
                name='TT66.BTV.660528',
                device_name='TT66.BTV.660528.DigiCam',
                filter_channel='TT66.BTV.660528.Filter',
                screen_channel='TT66.BTV.660528.Screen'
            )
        ]
        self.BctDevices = [
        ...
        if test_vistar:
            self.extraction_test(self.BtvDevices[0], self.BtvDevices[1], self.BctDevices[0])
            return
...
## defining the function to run the file -- the arguments give the user choices about data storage and the logbook
def main():
